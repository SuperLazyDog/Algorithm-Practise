...
void showResult(double t, double x, double y) {
	printf("%.15g %.15g %.15g\n", t, x, y);
	// printf("%.15g %.15g\n", t, x); // t-x
	// printf("%.15g %.15g\n", t, y); // t-y
	// printf("%.15g %.15g\n", x, y); // x-y
...
int main()
{
	...
	t = T_START;
	dt = DT;
	showResult(t, x[0], x[1]);
	// printf("%.15g %.15g %.15g\n", t, x[0], x[1]);

	while (t < T_END) {
		func(x, t, k1);

		v_s_mul(k1, 0.5 * dt, tmp, 2);
		v_v_add(x, tmp, tmp, 2);
		func(tmp, t + dt/2., k2);

		v_s_mul(k2, 0.5 * dt, tmp, 2);
		v_v_add(x, tmp, tmp, 2);
		func(tmp, t + dt/2., k3);
		...
		v_s_mul(k1, 1./6. * dt, tmp, 2);
		v_v_add(x, tmp, x, 2);

		v_s_mul(k2, 1./3. * dt, tmp, 2);
		v_v_add(x, tmp, x, 2);

		v_s_mul(k3, 1./3. * dt, tmp, 2);
		v_v_add(x, tmp, x, 2);

		v_s_mul(k4, 1./6. * dt, tmp, 2);
		v_v_add(x, tmp, x, 2);
